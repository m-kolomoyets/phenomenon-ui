        ...
        return (
            ...
            >
                <div className={clsx(s['trigger-wrap'], 'truncate')}>
                    <SelectTrigger ref={ref} className={clsx(s.trigger, 'truncate')} data-testid="select-trigger">
                        <SelectValue placeholder={placeholder} asChild>
                            <span>{value?.label}</span>
                        </SelectValue>
                        <SelectIcon className={s.icon}>{/* NOTE: Icon component here */}</SelectIcon>
                    </SelectTrigger>
                    {!rest?.disabled && isCleanable && value?.value ? (
                        <button className={s.clear} data-testid="select-clear" onClick={clearValueHandler}>
                    ...
                    <SelectContent className={s.content} position="popper" sideOffset={8}>
                        ...
                        <SelectViewport className={s.viewport}>
                            {totalRows ? (
                                ...
                                >
                                    <span className={clsx(s['empty-text'], 'truncate')} title={noOptionsMessage}>
                                        {noOptionsMessage}
                                    </span>
                                </div>
                            )}
